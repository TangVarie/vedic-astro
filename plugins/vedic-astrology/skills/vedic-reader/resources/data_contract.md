...
时间可信度: [高 / 中 / 低]
信号解释置信度: [高 / 中 / 低]
信号修正日志: [验前事未命中项 / 信号调整;无则"无"]
```

> ⚠️ **v3.5 补**（供下游 6 个解读引擎读取——core/career/love/soul/timing/rectifier 共约 47 处依赖）：
> - **时间可信度**：出生时间是否可靠，可由「有效精度」派生（±分钟级/±5分钟→高，±15分钟→中，±1小时/不确定→低）。
> - **信号解释置信度** 与 **信号修正日志**：由 reader 的**验前事阶段**写入（见 SKILL.md 阶段 3 / WRITE 3）——
>   验前事命中率高→信号解释置信度=高；未命中条目记入信号修正日志（无则写"无"）。
> - 下游读取规则示例：`时间可信度=高 + 信号解释置信度=低` → 触发"验前事复盘"，归因为规则在本盘适配度问题，而非时间问题。
