...
11. 回填元信息两维度（v3.5 补，供下游 6 引擎读取，约 47 处依赖）：
    把以下两个标签写回（或更新）structured_data.md「元信息」段——
    - 时间可信度: 由「有效精度」派生（±分钟级/±5分钟→高，±15分钟→中，±1小时/不确定→低）
    - 信号解释置信度: 由本步「验前事校准率」派生（≥4/5→高，2~3/5→中，≤1/5→低）
    ⚠️ 字段定义与 resources/data_contract.md「元信息」段一致；
       下游据 `时间可信度=高 + 信号解释置信度=低` 触发"验前事复盘"。
...
检查清单：
  ...
  □ 元信息两维度回填：时间可信度 + 信号解释置信度（第3次写入/本步骤，v3.5）
  □ 当前过运位置（Step 8.5写入）
→ 全部存在 → 继续
→ 有缺失 → 补写缺失部分
```
